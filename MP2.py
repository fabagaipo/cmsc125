...
class Gantt:
  def __init__(
    self,
    ganttProcesses: List[Process],
    processes: List[Process],
    avgWaiting: int,
    avgTurnaround: int,
  ):
    self._processes: List[Process] = processes
    self._ganttProcesses = ganttProcesses
    self._avgWaiting: int = avgWaiting
    self._avgTurnaround: int = avgTurnaround

  def print(self):
    greatestLength = max(
      process.turnaround() - (0 if index == 0 else self._ganttProcesses[index - 1].turnaround()) for index, process in enumerate(self._ganttProcesses)
    )
...
# returns waiting time and avg computing time for each process
def displayTime(processes: List[Process], key: Callable) -> Gantt:
...
def fcfs(processes) -> Gantt:
  return displayTime(processes, lambda x: x.id())
...
def priority(processes) -> Gantt:
  return displayTime(processes, cmp_to_key(priorityCompare))
...
def sjf(processes) -> Gantt:
  return displayTime(processes, cmp_to_key(sjfCompare))
